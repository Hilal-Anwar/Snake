...
public class SnakeBoardLayout implements Initializable, SnakeData {
	static AudioClip killed = new AudioClip(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("Untitled 2.wav")).toString());
	static AudioClip start = new AudioClip(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("progress_start.wav")).toString());
	static AudioClip end = new AudioClip(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("progress_end.wav")).toString());
	static AudioClip point = new AudioClip(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("big_point.wav")).toString());
	final Image BACKGROUND_IMAGE = new Image(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("image.jpg")).toString());
	final Image BIG_FOOD = new Image(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("big_food.png")).toString());
	final Image FOOD = new Image(Objects.requireNonNull(SnakeBoardLayout.class.getResource("food.png")).toString());
	final Image SNAKE_BODY = new Image(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("snakeimage.png")).toString());
	static AudioClip Background_Sound = new AudioClip(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("background_sound.mp3")).toString());
	static int speed = 7;
	int diff_x, diff_y;
	static int WIDTH = 37;
	...
	AudioClip eating_sound = new AudioClip(
			Objects.requireNonNull(SnakeBoardLayout.class.getResource("Untitled 1.wav")).toString());
	@FXML
	Canvas game_board;
	@FXML
	...
	private void showAbout() throws IOException {
		Scene scene = new Scene(FXMLLoader.load(Objects.requireNonNull(getClass().getResource("About.fxml"))), 903,
				489);
	...
	public void DrawSnake_board_on_Canvas(GraphicsContext gc) throws IOException {
		...
		App.scene.widthProperty().addListener(observable -> {
			game_board.setWidth(App.scene.getWidth());
			top_image.setFitWidth(App.scene.getWidth());
			bottom_image.setFitWidth(App.scene.getWidth());
			diff_x = ((int) (game_board.getWidth() / cornerSize)) - WIDTH;
		});
		App.scene.heightProperty().addListener(observable -> {
			game_board.setHeight(App.scene.getHeight() - 116);
		...
		if (gameOver) {
			...
			if (condition && SCORE > 0) {
				...
				if (!x.isEmpty()) {
					System.out.println(new ArrayList<>(x.keySet().stream().sorted(Comparator.naturalOrder()).collect(Collectors.toList())));
					if ((new ArrayList<>(x.keySet().stream().sorted(Comparator.naturalOrder()).collect(Collectors.toList())).get(0) < SCORE)
							|| x.size() < 5) {
						snake_Master();
					}
				} else {
		...
		gc.drawImage(BACKGROUND_IMAGE, 0, 0, App.scene.getWidth(), App.scene.getHeight() - 2 * 58);
		...
		for (int i = 0; i < snake.size(); i++) {
			...
			if (i == 0)
				gc.drawImage(
						new Image(
								Objects.requireNonNull(SnakeBoardLayout.class.getResource(HEARD_DIRECTION)).toString()),
	...
	void snake_Master() throws IOException {
		Scene scene1 = new Scene(FXMLLoader.load(Objects.requireNonNull(getClass().getResource("dialogBox.fxml"))), 288,
				191);
	...
	void showHighScore() throws IOException {
		Scene scene = new Scene(FXMLLoader.load(Objects.requireNonNull(getClass().getResource("scoreBoard.fxml"))), 462,
				322);
		HIGH_SCORE_STAGE.setTitle("Snake");
		HIGH_SCORE_STAGE.setScene(scene);
		HIGH_SCORE_STAGE.getIcons().add(e);
